...
def should_use_noninteractive():
    """Check if we should use non-interactive backend."""
    # Explicit environment variable to force non-interactive
    if os.environ.get('MATPLOTLIB_BACKEND', '').lower() == 'agg':
        return True
    if os.environ.get('NONINTERACTIVE', '').lower() in ('1', 'true', 'yes'):
        return True
    # CI environment
...
try:
    # Force non-interactive mode if requested
    if should_use_noninteractive():
        mpl.use('Agg')
...
def show_plot(block=False, save_path=None):
    ...
    try:
        ...
        # Only show plot if not in non-interactive mode and display available
        if not should_use_noninteractive() and has_display():
